...
class CertManager(object):
    ...
    def _create_subject(self, cert_info):
        attributes = []
        if cert_info.country: attributes.append(x509.NameAttribute(x509.oid.NameOID.COUNTRY_NAME, str(cert_info.country)))
        if cert_info.state: attributes.append(x509.NameAttribute(x509.oid.NameOID.STATE_OR_PROVINCE_NAME, str(cert_info.state)))
        if cert_info.city: attributes.append(x509.NameAttribute(x509.oid.NameOID.LOCALITY_NAME, str(cert_info.city)))
        if cert_info.owner: attributes.append(x509.NameAttribute(x509.oid.NameOID.ORGANIZATION_NAME, str(cert_info.owner)))
        if cert_info.organizational_unit: attributes.append(x509.NameAttribute(x509.oid.NameOID.ORGANIZATIONAL_UNIT_NAME, str(cert_info.organizational_unit)))
    ...
    def new_server_cert(self, server_info, server_key, ca_cert, ca_pk, valid_days = 729):
        ...
        cert_builder = x509.CertificateBuilder(
            ...
            x509.DNSName(str(server_info.alt_dns_name))]), critical=False
